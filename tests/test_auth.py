...
# --- PyJWT 2.x: jwt.encode() returns str ---

def test_jwt_encode_returns_str():
    token = jwt.encode({"sub": "user1", "exp": int(time.time()) + 3600}, JWT_SECRET, algorithm=JWT_ALGORITHM)
    assert isinstance(token, str)


def test_jwt_encode_str_format():
    token = jwt.encode({"sub": "user1", "exp": int(time.time()) + 3600}, JWT_SECRET, algorithm=JWT_ALGORITHM)
    assert "." in token
    parts = token.split(".")
    assert len(parts) == 3
...
def test_jwt_decode_expired_no_verify():
...
# --- PyJWT 2.x: exceptions with algorithms parameter ---
